(function () {
    ...
    if (!root || !window.knowledgeWorkspace) return;
    ...
    let current = window.knowledgeWorkspace;
    let path = [];
    let viewMode = 'grid';
    ...
    function renderBreadcrumb() {
        ...
        const parts = [{ name: 'База знаний', node: window.knowledgeWorkspace }].concat(path);
        ...
        parts.forEach((part, index) => {
            ...
            button.addEventListener('click', () => {
                const nextPath = index === 0 ? [] : path.slice(0, index);
                openFolder(part.node, nextPath);
            });
    ...
    function renderItems() {
        ...
        items.querySelectorAll('[data-item-index]').forEach(button => {
            button.addEventListener('click', () => {
                const node = visible[Number(button.dataset.itemIndex)];
                if (node.type === 'folder') {
                    openFolder(node, path.concat([{ name: node.name, node: node }]));
                } else {
                    openFile(node);
                }
            });
    ...
    function openFile(file) {
        ...
        documentUpdated.textContent = 'Изменён ' + file.updated;
        documentContent.innerHTML = markdown(file.content);
    ...
    upButton.addEventListener('click', () => {
        ...
        const parent = nextPath.length === 0 ? window.knowledgeWorkspace : nextPath[nextPath.length - 1].node;
        openFolder(parent, nextPath);
    });
