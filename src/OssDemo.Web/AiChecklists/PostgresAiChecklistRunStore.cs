...
internal sealed class PostgresAiChecklistRunStore(IConfiguration configuration) : IAiChecklistRunStore
{
    ...
    public async Task<AiChecklistRunState> CreateAsync(FacilityProfile profile, Guid facilityId, string facilityName, IReadOnlyList<AiChecklistEvidence> evidence, IReadOnlyList<AiChecklistBatchPlan> batches, CancellationToken cancellationToken, AiChecklistRunSnapshot? snapshot = null, Guid? draftId = null)
    {
        ...
        {
            await using var insert = new NpgsqlCommand("INSERT INTO app_ai_checklist_batches(run_id,batch_index,topic,evidence_ids,criterion_codes,applicability_reason,search_query,fallback_title,classifier_section,requirement_ids) VALUES(@runId,@index,@topic,@ids,@codes,@reason,@query,@fallback,@section,@requirementIds)", connection, transaction);
            ...
            insert.Parameters.AddWithValue("requirementIds", batch.RequirementIds?.ToArray() ?? []);
            await insert.ExecuteNonQueryAsync(cancellationToken);
        }
    ...
    public async Task<AiChecklistRunState?> GetAsync(Guid runId, CancellationToken cancellationToken)
    {
        ...
        await using (var batchCommand = new NpgsqlCommand("SELECT batch_index,topic,evidence_ids,status,items::text,error,duration_ms,updated_at,criterion_codes,applicability_reason,search_query,fallback_title,classifier_section,evidence::text,stage,stage_message,found_source_count,draft_output,requirement_ids FROM app_ai_checklist_batches WHERE run_id=@id ORDER BY batch_index", connection))
        {
    ...
    public async Task<bool> QueueBatchesAsync(Guid runId, IReadOnlyList<int> batchIndexes, CancellationToken cancellationToken)
    {
        ...
        await using var command = new NpgsqlCommand("UPDATE app_ai_checklist_batches SET status=CASE WHEN status IN ('pending','failed') OR (status='completed' AND jsonb_array_length(items)=0) THEN 'queued' ELSE status END,error=CASE WHEN status IN ('pending','failed') OR (status='completed' AND jsonb_array_length(items)=0) THEN NULL ELSE error END,stage=CASE WHEN status IN ('pending','failed') OR (status='completed' AND jsonb_array_length(items)=0) THEN 'queued' ELSE stage END,stage_message=CASE WHEN status IN ('pending','failed') OR (status='completed' AND jsonb_array_length(items)=0) THEN 'Ожидает последовательной обработки' ELSE stage_message END,draft_output=CASE WHEN status IN ('pending','failed') OR (status='completed' AND jsonb_array_length(items)=0) THEN '' ELSE draft_output END,found_source_count=CASE WHEN status IN ('pending','failed') OR (status='completed' AND jsonb_array_length(items)=0) THEN 0 ELSE found_source_count END,updated_at=now() WHERE run_id=@runId AND batch_index=ANY(@indexes)", connection, transaction);
        command.Parameters.AddWithValue("runId",runId);
    ...
    public async Task<bool> StopAsync(Guid runId, CancellationToken cancellationToken)
    {
        ...
        await using (var skip = new NpgsqlCommand("UPDATE app_ai_checklist_batches SET status='skipped',stage='skipped',stage_message='Остановлено до начала обработки',error='Остановлено пользователем.',updated_at=now() WHERE run_id=@runId AND status IN ('pending','queued')", connection, transaction))
        {
    ...
    public async Task<AiChecklistBatchWork?> ClaimNextBatchAsync(CancellationToken cancellationToken)
    {
        ...
        var batchIndex = 0;
        await using (var connection = await OpenAsync(cancellationToken))
        await using (var claim = new NpgsqlCommand("WITH next_batch AS (SELECT batch.run_id,batch.batch_index FROM app_ai_checklist_batches batch JOIN app_ai_checklist_runs run ON run.id=batch.run_id WHERE batch.status='queued' AND run.status='ready' ORDER BY run.updated_at,batch.batch_index FOR UPDATE OF batch SKIP LOCKED LIMIT 1), claimed AS (UPDATE app_ai_checklist_batches batch SET status='running',stage='searching',stage_message='Ищем основания в базе знаний',updated_at=now() FROM next_batch WHERE batch.run_id=next_batch.run_id AND batch.batch_index=next_batch.batch_index RETURNING batch.run_id,batch.batch_index) UPDATE app_ai_checklist_runs run SET updated_at=now() FROM claimed WHERE run.id=claimed.run_id RETURNING claimed.run_id,claimed.batch_index", connection))
    ...
    public async Task UpdateProgressAsync(Guid runId,int batchIndex,string stage,string message,int foundSourceCount,string draftOutput,CancellationToken ct){await using var connection=await OpenAsync(ct);await using var command=new NpgsqlCommand("UPDATE app_ai_checklist_batches SET stage=@stage,stage_message=@message,found_source_count=@sourceCount,draft_output=@draft,updated_at=now() WHERE run_id=@runId AND batch_index=@index AND status='running'",connection);command.Parameters.AddWithValue("stage",stage);command.Parameters.AddWithValue("message",message);command.Parameters.AddWithValue("sourceCount",foundSourceCount);command.Parameters.AddWithValue("draft",draftOutput);command.Parameters.AddWithValue("runId",runId);command.Parameters.AddWithValue("index",batchIndex);await command.ExecuteNonQueryAsync(ct);}
    ...
    public async Task CompleteCriterionAsync(Guid runId,int batchIndex,IReadOnlyList<AiChecklistEvidence> evidence,IReadOnlyList<AiGeneratedChecklistItem> items,long durationMs,CancellationToken ct){await using var connection=await OpenAsync(ct);await using var command=new NpgsqlCommand("UPDATE app_ai_checklist_batches SET status='completed',stage=CASE WHEN @isTemplate THEN 'completed_template' WHEN jsonb_array_length(CAST(@items AS jsonb))>0 THEN 'completed_ai' ELSE 'completed_base' END,stage_message=CASE WHEN @isTemplate THEN 'Пункты утверждённого рабочего слоя проверены и сохранены' WHEN jsonb_array_length(CAST(@items AS jsonb))>0 THEN 'ИИ-формулировка проверена и сохранена' ELSE 'Сохранён базовый пункт классификатора' END,items=CAST(@items AS jsonb),evidence=CAST(@evidence AS jsonb),error=NULL,duration_ms=@duration,updated_at=now() WHERE run_id=@runId AND batch_index=@index; UPDATE app_ai_checklist_runs SET status='stopped',updated_at=now() WHERE id=@runId AND status='stopping' AND NOT EXISTS(SELECT 1 FROM app_ai_checklist_batches WHERE run_id=@runId AND status IN ('running','queued'))",connection);command.Parameters.AddWithValue("items",JsonSerializer.Serialize(items,JsonOptions));command.Parameters.AddWithValue("evidence",JsonSerializer.Serialize(evidence,JsonOptions));command.Parameters.AddWithValue("isTemplate",evidence.Any(item=>item.Id.StartsWith("TPL-",StringComparison.OrdinalIgnoreCase)||item.Id.StartsWith("CAT-",StringComparison.OrdinalIgnoreCase)));command.Parameters.AddWithValue("duration",durationMs);command.Parameters.AddWithValue("runId",runId);command.Parameters.AddWithValue("index",batchIndex);await command.ExecuteNonQueryAsync(ct);}
