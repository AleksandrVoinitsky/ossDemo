    ...
    CancellationToken cancellationToken) =>
{
    ...
    var messages = new List<InferenceMessage>
    {
        new("system", ChatPrompt.BuildSystemMessage(
            ...
            ChatClarification.Build(ragQuestion, matches.Count > 0)))
    };
...
internal static class ChatPrompt
{
    public static string BuildSystemMessage(string context, bool hasSources, IReadOnlyList<string> ambiguousDocuments, string? clarification)
    {
        ...
        : """
            ...
            В этом запросе нет подтверждающих фрагментов базы знаний. На приветствия и общие
            вопросы отвечай естественно, без предупреждений. Для общих понятий можно дать
            нейтральное объяснение из предметной области. Но факты о конкретных организациях,
            объектах, документах и требованиях допустимы только при подтверждающих источниках.
