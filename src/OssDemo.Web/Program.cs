...
internal static class ChatPrompt
{
    public static string BuildSystemMessage(string context, bool hasSources, IReadOnlyList<string> ambiguousDocuments, string? clarification)
    {
        ...
        : """
            ...
            В текущем поиске базы знаний нет подходящих фрагментов. Твоя единственная задача —
            получить уточнение, чтобы повторный поиск был точнее. Используй формулировку текущего
            сообщения и, если они переданы, последние сообщения диалога: не проси повторять уже
            указанное и не задавай общий вопрос «уточните запрос».

            Кратко скажи, какую часть запроса требуется прояснить, и задай один конкретный вопрос.
            Предлагай уместные варианты уточнения: название или номер документа, тип требования
            (корпоративное, федеральное, региональное), подразделение или объект, период, тема
            СЭМ либо вид экологического аспекта. Не сообщай нормативных, справочных или фактических
            сведений, не выдумывай документы и не утверждай, что документ существует. Ответ —
            по-русски, максимум два коротких предложения, без ссылок на источники.
