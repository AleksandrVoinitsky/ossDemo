internal static class AiChecklistAgentChecks
{
    ...
    public static async Task RunOrchestrationChecksAsync()
    {
        ...
        AssertEqual(94, catalogRunValue.Snapshot.ExpectedItemCount);
        ...
        AssertTrue(catalogRunValue.Snapshot.ItemTraces.All(item => item.Provenance == "requirements-registry-v2"),
            "Снимок должен состоять только из контрольных групп общего реестра требований.");
        ...
        AssertEqual(94, catalogRunValue.Batches.Sum(batch => batch.EvidenceIds.Count));
        ...
        AssertEqual(94, catalogChecklistValue.Items.Count);
