internal sealed class InMemoryAiChecklistRunStore : IAiChecklistRunStore
{
    ...
    public Task<bool> QueueBatchAsync(Guid runId, int batchIndex, CancellationToken cancellationToken)
    {
        lock (gate)
        {
            ...
            if (batch.Status is "completed" or "queued" or "running") return Task.FromResult(true);
    ...
    { lock (gate) { if (!runs.TryGetValue(runId, out var run) || run.Status != "ready" || run.Batches.Any(item => item.Status != "completed")) return Task.FromResult(false); runs[runId] = run with { Status = "finalizing", UpdatedAt = DateTimeOffset.UtcNow }; return Task.FromResult(true); } }
