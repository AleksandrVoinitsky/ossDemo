internal sealed class InMemoryAiChecklistRunStore : IAiChecklistRunStore
{
    ...
    { lock (gate) { if (!runs.TryGetValue(runId, out var run) || run.Status is not ("ready" or "finalizing") || run.Batches.Any(item => item.Status != "completed")) return Task.FromResult(false); runs[runId] = run with { Status = "finalizing", UpdatedAt = DateTimeOffset.UtcNow }; return Task.FromResult(true); } }
